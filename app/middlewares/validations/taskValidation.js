    ...
    .custom((value, helpers) => {
      ...
      if (value && value.length > maxSize) {
        return helpers.message(
          "El campo image debe ser una imagen válida de máximo 500 KB"
        );
      }
      return value;
    })
  ...
  people: Joi.array()
    .items(
      Joi.object({
        ...
        role_id: Joi.number().integer().allow(null, "").optional().empty(null),
...
const updatePointsAndTasksSchema = Joi.object({
  home_id: Joi.number().required().messages({
    "number.base": "El home_id debe ser un número",
    "any.required": "El home_id es requerido",
  }),  
  task_id: Joi.number().required().messages({
    "number.base": "El task_id debe ser un número",
    "any.required": "El task_id es requerido",
  }),
  people: Joi.array()
    .items(
      Joi.object({
        id: Joi.number().integer().allow(null, "").optional().empty(null),
        person_id: Joi.number().required().messages({
          "number.base": "El person_id debe ser un número",
          "any.required": "El person_id es requerido",
        }),
    ...
    .required()
    .messages({
      "array.base": "people debe ser un array",
      "any.required": "people es requerido",
    }),
});
