...
const AuthController = {
    ...
    try {
      ...
      if (!user) {
        ...
        if (user) {
          ...
          if (image) {
            imageUpdateNeeded = true;
            imageUpdateData = { imageUrl: image, personId: person.id };
          }
        }
      } else {
        person = user.person;
  ...
  async googleCallbackAPK(req, res) {
    logger.info("datos recibidos al registrarse o loggin con google APK");
    logger.info(JSON.stringify(req.body));
    const t = await sequelize.transaction();
    try {
  ...
  async handleImageUpdate({ imageUrl, personId }) {
    try {
      ...
      logger.info(`Imagen actualizada para persona ${personId}`);
    } catch (error) {
      logger.error(
        `Error actualizando imagen para persona ${personId}:`,
        error
      );
    }
  },
  ...
  async saveImageFromUrl(imageUrl, personId) {
    try {
      ...
      // Usar pipeline para mejor manejo de streams
