...
const PersonHomeWarehouseProductController = {
  ...
  async index(req, res) {
    ...
    try {
      ...
      logger.error(
        "Error en PersonHomeWarehouseProductController->index: " + errorMsg
      );
  ...
  async personHomeWarehouseProducts(req, res) {
    logger.info(
      `${req.user.name} - Consulta de los productos en un almacén específicode una persona en el hogar`
    );
    ...
    try {
      ...
      if (!home) {
        logger.error(
          `PersonHomeWarehouseController->homeWarehouseProducts: Hogar no encontrado con ID ${home_id}`
        );
      ...
      if (!warehouse) {
        logger.error(
          `PersonHomeWarehouseController->homeWarehouseProducts: Almacén no encontrado con ID ${warehouse_id}`
        );
        return res.status(204).json({ msg: "HomeWarehouseNotFound" });
      }

      // Buscar los productos específicos en el almacén del hogar
      const homeWarehouseProducts =
      ...
      const result = homeWarehouseProducts.map((homeWarehouseProduct) => ({
        ...
        productId: homeWarehouseProduct.product_id,
        product_id: homeWarehouseProduct.product_id,
        productName: homeWarehouseProduct.product.name, // Relación con el producto, // Relación con el producto
        name: homeWarehouseProduct.product.name, // Relación con el producto
        categoryId: homeWarehouseProduct.product.category_id,
        category_id: homeWarehouseProduct.product.category_id,
        nameCategory: homeWarehouseProduct.product.category.name, // Lógica para obtener traducción de la categoría
      ...
      const errorMsg = error.details
        ...
        : error.message || "Error desconocido";
      logger.error(
        "Error en PersonHomeWarehouseController->homeWarehouseProducts: " +
          errorMsg
      );
      res.status(500).json({ error: "ServerError", details: errorMsg });
    }
  ...
  async store(req, res) {
    ...
    if (!home) {
      logger.error(
        `PersonHomeWarehouseController->store: Hogar no encontrado con ID ${home_id}`
      );
      return res.status(204).json({ msg: "HomeNotFound" });
    }
    ...
    if (!person) {
      logger.error(
        `PersonHomeWarehouseController->store: La persona con ID ${person_id} no está asociada con el hogar con ID ${home_id}`
      );
      return res.status(204).json({ msg: "PersonNotAssociatedWithHome" });
    }
    ...
    if (!warehouse) {
      logger.error(
        `PersonHomeWarehouseController->store: Almacén no encontrado con ID ${warehouse_id}`
      );
    ...
    if (!status) {
      logger.error(
        `PersonHomeWarehouseController->store: Estado no encontrado con ID ${status_id}`
      );
    ...
    if (category_id && category_id !== 0) {
      ...
      if (!category) {
        logger.error(
          `PersonHomeWarehouseController->store: Categoría no encontrada con ID ${category_id}`
        );
    ...
    try {
      ...
      if (!product) {
        logger.info(
          "PersonHomeWarehouseController->store: Creando nuevo producto"
        );
      ...
      const personHomeWarehouseProduct = await PersonProductRepository.create(
        req.body,
        req.file,
        t,
        person_id
      ...
      res
        .status(201)
        .json({ personHomeWarehouseProduct: personHomeWarehouseProduct });
    } catch (error) {
      ...
      const errorMsg = error.details
        ...
        : error.message || "Error desconocido";
      logger.error(
        "Error en PersonHomeWarehouseProductController->store: " + errorMsg
      );
      res.status(500).json({ error: "ServerError", details: errorMsg });
    }
  },
  ...
  async show(req, res) {
    logger.info(
      `${req.user.name} - Consulta del producto específico en un almacén para el hogar de una persona`
    );

    try {
      // Buscar el producto específico en el almacén del hogar de la persona
      const homeWarehouseProduct = await PersonProductRepository.findById(
        req.body.id
      );

      if (!homeWarehouseProduct) {
        logger.error(
          `PersonHomeWarehouseProductController->show: No se encontro el producto en el alamcen con id: ${req.body.id}`
        );
        return res
          .status(204)
  ...
  async update(req, res) {
    logger.info(
      `${req.user.name} - Inicia actualización de relación en person_home_warehouse_products`
    );
    ...
    const personHomeWarehouseProduct = await PersonProductRepository.findById(
      id
    );
    ...
    try {
      ...
      if (home_id) {
        ...
        if (!home) {
          logger.error(
            `PersonHomeWarehouseProductsController->update: Hogar no encontrado con ID ${home_id}`
          );
          return res.status(204).json({ msg: "HomeNotFound" });
        }
      }
      ...
      if (product_id) {
        ...
        if (!product) {
          logger.error(
            `PersonHomeWarehouseProductsController->update: Producto no encontrado con ID ${product_id}`
          );
          return res.status(204).json({ msg: "ProductNotFound" });
        }
      } else {
        product = await ProductRepository.findById(
          personHomeWarehouseProduct.product_id
        );
      }
      ...
      if (status_id) {
        ...
        if (!status) {
          logger.error(
            `PersonHomeWarehouseProductsController->update: Estado no encontrado con ID ${status_id}`
          );
          return res.status(204).json({ msg: "StatusNotFound" });
        }
      }
      ...
      const personHomeWarehouseProductUpdate =
        await PersonProductRepository.update(
          personHomeWarehouseProduct,
          req.body,
          req.file,
          t
      ...
      return res
        .status(200)
        .json({ personHomeWarehouseProduct: personHomeWarehouseProductUpdate });
    } catch (error) {
      await t.rollback();
      logger.error(
        `Error en PersonHomeWarehouseProductsController->update: ${error.message}`
      );
      return res
        .status(500)
  ...
  async destroy(req, res) {
    logger.info(
      `${req.user.name} - Eliminando un producto de un almacen en un hogar`
    );
    ...
    try {
      ...
      if (!personHomeWarehouseProduct) {
        logger.error(
          `PersonHomeWarehouseProductsController->destroy: Producto no encontrado con ID ${req.body.id}`
        );
        return res
          .status(204)
          .json({ msg: "PersonHomeWarehouseProductNotFound" });
      ...
      const errorMsg = error.message || "Error desconocido";
      logger.error(
        `PersonHomeWarehouseProductsController->destroy: ${errorMsg}`
      );
      res.status(500).json({ error: "ServerError", details: errorMsg });
    }
  },
